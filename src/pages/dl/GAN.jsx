...
function GAN() {
  ...
  const generateSampleData = () => {
    ...
    // Added n_clusters mapping!
    const clusters = sampleDataType === 'blobs' ? 3 : 2;

    axios.post(`${apiUrl}/gan/sample_data`, {
  ...
  const trainModel = () => {
    if (points.length < 5) { setError("Place at least 5 points."); return; }
    setLoading(true); setError(null);
    axios.post(`${apiUrl}/gan/train`, { points: points, parameters: { epochs, learningRate } })
    .then(response => {
    ...
    .catch(err => setError("Failed to run GAN."))
    .finally(() => setLoading(false));
  };
  ...
  useEffect(() => {
    ...
    points.forEach(point => {
      ...
      ctx.fillStyle = '#22c55e'; ctx.fill(); ctx.strokeStyle = 'black'; ctx.lineWidth = 1; ctx.stroke();
    });
  }, [points]);
    ...
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
      <div style={{ backgroundColor: 'white', padding: '2rem', borderRadius: '0.5rem', width: '90%', maxWidth: '500px', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
          <h2 style={{ fontSize: '1.2rem', fontWeight: '600' }}>Generate Real Data</h2>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', marginTop: '1.5rem' }}>
          ...
          <button onClick={generateSampleData} style={{ padding: '0.6rem 1.2rem', backgroundColor: '#3b82f6', color: 'white', cursor: 'pointer', borderRadius: '0.5rem', border: 'none', fontWeight: '500' }}>Generate Real Data</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <motion.div className="model-page" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <div className="model-header">
        ...
        <h1 className="model-title">Generative Adversarial Networks (GANs) </h1>
      </div>

      <p className="model-description">GANs use two competing networks. The <strong>Generator</strong> creates fake data, and the <strong>Discriminator</strong> tries to catch the fakes. By fighting each other, the Generator learns to create perfectly realistic data!<InfoButton algoId="gan" /></p>
      ...
      <div className="content-container" style={{ display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '1.5rem', marginBottom: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <h2 className="section-title">Adversarial Controls</h2>
            <div style={{ backgroundColor: 'white', padding: '1.5rem', borderRadius: '6px', border: '1px solid #e5e7eb', marginBottom: '1.5rem' }}>
              ...
              <div style={{ padding: '1rem', backgroundColor: '#f8fafc', borderRadius: '6px', border: '1px dashed #cbd5e1' }}>
                  <h4 style={{ margin: '0 0 0.5rem 0', color: '#334155' }}>The Architecture</h4>
                  <p style={{ margin: 0, fontSize: '0.85rem', color: '#64748b', lineHeight: '1.5' }}>
                      <strong>Generator:</strong> Starts with pure static Noise &rarr; Hidden Layer &rarr; Outputs 2D Fake Dots.<br/>
                      <strong>Discriminator:</strong> Takes 2D Dots &rarr; Hidden Layer &rarr; Outputs Probability (0=Fake, 1=Real).
                  </p>
              </div>
            </div>
        ...
        {results && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ backgroundColor: 'white', padding: '1.5rem', borderRadius: '6px', border: '1px solid #e5e7eb', marginBottom: '2rem' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '2rem' }}>
                    <div style={{ padding: '1.5rem', border: '2px solid #a855f7', borderRadius: '8px', backgroundColor: '#f9fafb' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                            <h4 style={{ fontWeight: '600', margin: 0 }}>Generator Evolution</h4>
                            <span style={{ backgroundColor: '#f3e8ff', color: '#a855f7', padding: '4px 10px', borderRadius: '12px', fontWeight: '600' }}>Epoch: {playback.frames[playback.currentIndex]?.epoch}</span>
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
                            <button onClick={togglePlayback} style={{ backgroundColor: '#a855f7', color: 'white', border: 'none', borderRadius: '4px', width: '40px', height: '40px', cursor: 'pointer' }}>{playback.isPlaying ? '⏸' : '▶'}</button>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                        <div style={{ backgroundColor: '#f3f4f6', padding: '1.5rem', borderRadius: '8px', border: '1px solid #e5e7eb', textAlign: 'center' }}>
                            <p style={{ fontSize: '1.2rem', fontWeight: '600', margin: 0 }}>Final Disc. Loss: {results.final_d_loss.toFixed(4)}</p>
                            <p style={{ color: '#4b5563', margin: '0.5rem 0 0 0' }}>Final Gen. Loss: {results.final_g_loss.toFixed(4)}</p>
                        </div>
                        <div style={{ flex: 1, padding: '1.5rem', border: '1px solid #e5e7eb', borderRadius: '8px', backgroundColor: '#ffffff' }}>
                            ...
                            <p style={{ color: '#4b5563', fontSize: '0.95rem', lineHeight: '1.6' }}>
                                Watch the animation! The Generator starts by vomiting pure purple static. Every time the Discriminator catches it, the Generator mathematically adjusts its weights to fool the Discriminator better next time.<br/><br/>
                                By the end, the purple dots perfectly overlap the green dots!
                            </p>
                        </div>
                    </div>

                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        <h4 style={{ marginBottom: '1rem', fontWeight: '600' }}>Min-Max Loss War</h4>
                        <div style={{ height: 250, width: '100%' }}>
                            <ResponsiveContainer>
                                <LineChart data={results.loss_history}>
                                    ...
                                    <Tooltip />
                                    <Legend />
                                    <Line type="monotone" dataKey="d_loss" stroke="#22c55e" name="Discriminator Loss" strokeWidth={2} dot={false} />
                                    <Line type="monotone" dataKey="g_loss" stroke="#a855f7" name="Generator Loss" strokeWidth={2} dot={false} />
                                </LineChart>
                            </ResponsiveContainer>
                        </div>
                        <p style={{fontSize: '0.8rem', color: '#6b7280', textAlign: 'center'}}>Notice how the lines fight! When D-Loss drops (Catches fakes), G-Loss spikes (Gets caught).</p>
                    </div>

                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        <h4 style={{ marginBottom: '1rem', fontWeight: '600' }}>The Discriminator's Final Heatmap</h4>
                        <div style={{ height: 250, width: '100%' }}>
                            <img src={`data:image/png;base64,${results.disc_map}`} alt="Heatmap" style={{ width: '100%', height: '100%', objectFit: 'contain', borderRadius: '4px', border: '1px solid #e5e7eb' }} />
                        </div>
                        <p style={{fontSize: '0.8rem', color: '#6b7280', textAlign: 'center'}}>
                            The Deep Green areas are where the Discriminator is 100% sure the data is Real. Purple is Fake!
                        </p>
                    </div>
                </div>
            </motion.div>
