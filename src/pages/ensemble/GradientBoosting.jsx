...
function GradientBoosting() {
  ...
  const generateSampleDataWithOptions = () => {
    ...
    axios.post(`${apiUrl}/gb/sample_data`, {
      type: activeTab,
    ...
    .catch(err => {
        setError(`Network Error: Failed to fetch sample data.`);
    })
  ...
  const trainModel = () => {
    ...
    const requestData = {
      ...
      type: activeTab,
      parameters: { estimators: estimators, learningRate: learningRate, max_depth: maxDepth }
    };

    axios.post(`${apiUrl}/gb/train`, requestData)
      .then(response => {
  ...
  const handlePrediction = () => {
    ...
    axios.post(`${apiUrl}/gb/predict`, requestData)
    .then(response => {
      if (response.data.error) { setError(response.data.error); return; }
  ...
  return (
    <motion.div className="model-page" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
      ...
      <div className="model-header">
        ...
        <h1 className="model-title">Gradient Boosting </h1>
      </div>

      <p className="model-description">
        Gradient Boosting is a powerful ensemble method that builds trees sequentially. Unlike Random Forest which builds independent trees, each new tree in Gradient Boosting is trained to correct the errors (residuals) of the previous ones.
        <InfoButton algoId="gb" />
      </p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '1.5rem', width: '100%', marginBottom: '1.5rem' }}>
          ...
          <div style={{ width: '100%', gridColumn: '1 / 2', display: 'flex', flexDirection: 'column' }}>
            <div className="section-header">
              <h2 className="section-title">Gradient Boosting {activeTab === 'classification' ? 'Classification' : 'Regression'}</h2>
          ...
          <div style={{ width: '100%', gridColumn: '2 / 3', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ marginBottom: '1.5rem', backgroundColor: 'white', padding: '1.5rem', borderRadius: '6px', border: '1px solid #e5e7eb', width: '100%' }}>
              ...
              <div className="form-group" style={{ marginBottom: '1.5rem' }}>
                <label style={{ display: 'block', marginBottom: '0.75rem', fontWeight: '500', color: '#4b5563', fontSize: '1rem' }}>Total Estimators (Trees): {estimators}</label>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: '#6b7280', marginTop: '0.5rem' }}>
                  <span>1 Tree (Weak)</span><span>100 Trees (Strong)</span>
                </div>
              </div>
              ...
              <div className="form-group" style={{ marginBottom: '1.5rem' }}>
                ...
                <input type="range" min="1" max="5" step="1" value={maxDepth} onChange={(e) => setMaxDepth(parseInt(e.target.value))} style={{ width: '100%' }} />
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: '#6b7280', marginTop: '0.5rem' }}>
                  <span>1 (Stump)</span><span>5 (Deep)</span>
                </div>
              </div>
              ...
              <div className="form-group" style={{ marginBottom: '1.5rem' }}>
                ...
                <input type="range" min="0.01" max="1.0" step="0.01" value={learningRate} onChange={(e) => setLearningRate(parseFloat(e.target.value))} style={{ width: '100%' }} />
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: '#6b7280', marginTop: '0.5rem' }}>
                  <span>0.01 (Slow, Stable)</span><span>1.0 (Fast, Aggressive)</span>
                </div>
              </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '2rem' }}>
                    ...
                    <div style={{ padding: '1.5rem', border: '2px solid #3b82f6', borderRadius: '8px', backgroundColor: '#f9fafb' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                            ...
                            <span style={{ backgroundColor: '#eff6ff', color: '#3b82f6', padding: '4px 10px', borderRadius: '12px', fontWeight: '600', fontSize: '0.9rem' }}>
                                Trees: {playback.frames.length > 0 ? playback.frames[playback.currentIndex].estimators : results.estimators}
                            </span>
                        </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                        <div style={{ backgroundColor: '#f3f4f6', padding: '1.5rem', borderRadius: '8px', border: '1px solid #e5e7eb', textAlign: 'center' }}>
                            ...
                            <p style={{ color: '#4b5563', margin: '0.5rem 0 0 0' }}>
                                Training Score: {(results.final_train_score * 100).toFixed(2)}% | Trees Used: {results.estimators}
                            </p>
                        </div>
                        ...
                        <div style={{ flex: 1, padding: '1.5rem', border: '1px solid #e5e7eb', borderRadius: '8px', backgroundColor: '#ffffff', textAlign: 'center' }}>
                            ...
                            <p style={{ color: '#4b5563', fontSize: '0.95rem', lineHeight: '1.6', textAlign: 'left' }}>
                                Unlike Random Forests which average independent trees, Gradient Boosting calculates the <strong>mathematical residuals (errors)</strong> of the previous tree, and forces the new tree to specifically predict those errors.<br/><br/>
                                This makes it one of the most powerful algorithms on earth for tabular data.
                            </p>
                        </div>
                    </div>
                    ...
                    {results.history && results.history.length > 0 && (
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                            ...
                            <p style={{fontSize: '0.8rem', color: '#6b7280', textAlign: 'center', marginTop: '0.5rem'}}>
                                Training accuracy climbs rapidly as more trees are added to correct errors.
                            </p>
                        </div>
                    )}
                    ...
                    {results.feature_importance && results.feature_importance.length > 0 && (
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                            ...
                            <p style={{fontSize: '0.8rem', color: '#6b7280', textAlign: 'center', marginTop: '0.5rem'}}>
                                Shows which axis (Feature X1 or X2) the algorithm found most mathematically useful.
                            </p>
                        </div>
                    )}
                </div>
