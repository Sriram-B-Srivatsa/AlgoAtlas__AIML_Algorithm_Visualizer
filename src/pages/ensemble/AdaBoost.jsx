...
function AdaBoost() {
  ...
  const generateSampleDataWithOptions = () => {
    ...
    axios.post(`${apiUrl}/adaboost/sample_data`, {
      type: activeTab,
    ...
    .catch(err => {
        setError(`Network Error: Failed to fetch sample data. (${err.message})`);
    })
  ...
  const trainModel = () => {
    ...
    const requestData = {
      ...
      type: activeTab,
      parameters: { estimators: estimators, learningRate: learningRate }
    };

    axios.post(`${apiUrl}/adaboost/train`, requestData)
      .then(response => {
      ...
      .catch(err => setError("Failed to train model. Ensure backend is running."))
      .finally(() => setLoading(false));
  };
  ...
  const handlePrediction = () => {
    ...
    axios.post(`${apiUrl}/adaboost/predict`, requestData)
    .then(response => {
      if (response.data.error) { setError(response.data.error); return; }
    ...
    .catch(err => setError("Failed to make prediction."))
    .finally(() => setLoading(false));
  };
  ...
  return (
    <motion.div className="model-page" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
      ...
      <div className="model-header">
        ...
        <h1 className="model-title">AdaBoost </h1>
      </div>

      <p className="model-description">
        AdaBoost (Adaptive Boosting) is a sequential ensemble method. Instead of building independent trees, it builds "weak learners" (stumps) one by one. Each new stump mathematically focuses on fixing specific points that the previous stumps got wrong!
        <InfoButton algoId="adaboost" />
      </p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '1.5rem', width: '100%', marginBottom: '1.5rem' }}>
          ...
          <div style={{ width: '100%', gridColumn: '1 / 2', display: 'flex', flexDirection: 'column' }}>
            <div className="section-header">
              <h2 className="section-title">AdaBoost {activeTab === 'classification' ? 'Classification' : 'Regression'}</h2>
          ...
          <div style={{ width: '100%', gridColumn: '2 / 3', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ marginBottom: '1.5rem', backgroundColor: 'white', padding: '1.5rem', borderRadius: '6px', border: '1px solid #e5e7eb', width: '100%' }}>
              <h3 style={{ marginBottom: '1.25rem', fontSize: '1.1rem', fontWeight: '500' }}>Boosting Parameters</h3>

              <div className="form-group" style={{ marginBottom: '1.5rem' }}>
                <label style={{ display: 'block', marginBottom: '0.75rem', fontWeight: '500', color: '#4b5563', fontSize: '1rem' }}>Total Estimators (Stumps): {estimators}</label>
                <input type="range" min="1" max="100" step="1" value={estimators} onChange={(e) => setEstimators(parseInt(e.target.value))} style={{ width: '100%' }} />
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: '#6b7280', marginTop: '0.5rem' }}>
                  <span>1 Stump (Underfits)</span><span>100 Stumps (Fits perfectly)</span>
                </div>
              </div>

              <div className="form-group" style={{ marginBottom: '1.5rem' }}>
                <label style={{ display: 'block', marginBottom: '0.75rem', fontWeight: '500', color: '#4b5563', fontSize: '1rem' }}>Learning Rate: {learningRate.toFixed(2)}</label>
                <input type="range" min="0.01" max="2.0" step="0.01" value={learningRate} onChange={(e) => setLearningRate(parseFloat(e.target.value))} style={{ width: '100%' }} />
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: '#6b7280', marginTop: '0.5rem' }}>
                  <span>0.01 (Slow, needs more stumps)</span><span>2.0 (Fast, aggressive)</span>
                </div>
              </div>
            ...
            <div style={{ width: '100%', backgroundColor: 'white', padding: '1rem', borderRadius: '6px', border: '1px solid #e5e7eb', fontSize: '0.85rem' }}>
              <h3 style={{ marginBottom: '0.75rem', fontSize: '0.95rem', fontWeight: '500' }}>Bagging vs Boosting</h3>
              <div style={{ color: '#4b5563', lineHeight: '1.4' }}>
                <p style={{ marginBottom: '0.5rem' }}>Unlike Random Forest which trains deep trees in parallel (Bagging), AdaBoost trains shallow trees (stumps) sequentially (Boosting).</p>
                <ol style={{ paddingLeft: '1.25rem', marginBottom: '0.5rem' }}>
                  <li>Train a simple tree. It will make mistakes.</li>
                  <li>Increase the "weight" (importance) of the points it got wrong.</li>
                  <li>Train a new tree specifically focusing on those hard-to-predict points.</li>
                </ol>
              </div>
            </div>
          </div>
        ...
        {results && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} style={{ width: '100%', backgroundColor: 'white', padding: '1.5rem', borderRadius: '6px', border: '1px solid #e5e7eb', marginBottom: '2rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '2rem' }}>
                    ...
                    {/* (0,0) Boosting Boundary Evolution Animation */}
                    <div style={{ padding: '1.5rem', border: '2px solid #3b82f6', borderRadius: '8px', backgroundColor: '#f9fafb' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                            <h4 style={{ fontWeight: '600', margin: 0, color: '#1f2937' }}>Sequential Boundary Evolution</h4>
                            <span style={{ backgroundColor: '#eff6ff', color: '#3b82f6', padding: '4px 10px', borderRadius: '12px', fontWeight: '600', fontSize: '0.9rem' }}>
                                Stumps: {playback.frames.length > 0 ? playback.frames[playback.currentIndex].estimators : results.estimators}
                            </span>
                        </div>
                    ...
                    {/* (0,1) Final Stats with Boosting Insights */}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                        <div style={{ backgroundColor: '#f3f4f6', padding: '1.5rem', borderRadius: '8px', border: '1px solid #e5e7eb', textAlign: 'center' }}>
                            ...
                            <p style={{ color: '#4b5563', margin: '0.5rem 0 0 0' }}>
                                Training Score: {(results.final_train_score * 100).toFixed(2)}% | Stumps Used: {results.estimators}
                            </p>
                        </div>
                        ...
                        <div style={{ flex: 1, padding: '1.5rem', border: '1px solid #e5e7eb', borderRadius: '8px', backgroundColor: '#ffffff', textAlign: 'center' }}>
                            ...
                            <p style={{ color: '#4b5563', fontSize: '0.95rem', lineHeight: '1.6', textAlign: 'left' }}>
                                Notice the animation on the left. The first frame is literally just a single straight line (a Tree of Depth 1). <br/><br/>
                                By mathematically piling dozens of these straight lines on top of each other, and forcing the new lines to focus only on the mistakes of the old lines, AdaBoost can bend a smooth curve around highly complex data!
                            </p>
                        </div>
                    </div>
                    ...
                    {results.history && results.history.length > 0 && (
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                            <h4 style={{ marginBottom: '1rem', fontWeight: '600', color: '#1f2937', borderBottom: '2px solid #e5e7eb', paddingBottom: '0.5rem' }}>
                                Boosting Progress (Train vs Val)
                            </h4>
                            <div style={{ height: 250, width: '100%', marginTop: '1rem' }}>
                                <ResponsiveContainer>
                                    <LineChart data={results.history} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                                        ...
                                        <XAxis dataKey="estimators" tick={{fontSize: 12}} />
                            ...
                            <p style={{fontSize: '0.8rem', color: '#6b7280', textAlign: 'center', marginTop: '0.5rem'}}>
                                Training accuracy rapidly climbs as more stumps are added. Validation accuracy proves it generalizes well to unseen data.
                            </p>
                        </div>
                    )}
                    ...
                    {results.feature_importance && results.feature_importance.length > 0 && (
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                            ...
                            <p style={{fontSize: '0.8rem', color: '#6b7280', textAlign: 'center', marginTop: '0.5rem'}}>
                                Shows which axis (Feature X1 or X2) the algorithm found most mathematically useful for correcting mistakes.
                            </p>
                        </div>
                    )}
                </div>
