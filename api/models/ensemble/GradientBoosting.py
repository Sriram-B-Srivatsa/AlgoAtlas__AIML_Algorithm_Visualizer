...
def run_gb(data):
    try:
        ...
        for t in frames_to_render:
            if mode == 'classification':
                model = GradientBoostingClassifier(n_estimators=t, learning_rate=learning_rate, max_depth=max_depth, random_state=42)
                ...
                model = GradientBoostingRegressor(n_estimators=t, learning_rate=learning_rate, max_depth=max_depth, random_state=42)
...
def predict_gb_points(data):
    try:
        ...
        if mode == 'classification':
            model = GradientBoostingClassifier(n_estimators=estimators, learning_rate=learning_rate, max_depth=max_depth, random_state=42)
            model.fit(X_train, y_train.astype(str))
            predictions = model.predict(X_pred).tolist()
        else:
            model = GradientBoostingRegressor(n_estimators=estimators, learning_rate=learning_rate, max_depth=max_depth, random_state=42)
...
def generate_gb_sample_data(data):
    try:
        ...
        points = [{'x1': float(X[i][0]), 'x2': float(X[i][1]), 'y': y[i]} for i in range(len(X))]
